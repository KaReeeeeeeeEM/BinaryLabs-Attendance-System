...
const Form = () => {
    ...
    let handleSubmit = async (e) =>{
        e.preventDefault();
        if(formData.name == null || formData.password == null || formData.confirm == null){
  ...
  return (
    <div className='flex flex-col align-center justify-center w-screen h-screen bg-gradient-to-t from-green-200 to-green-300'>
        ...
        <form className="flex flex-col justify-center items-center md:w-1/3 h-[300px] m-auto" onSubmit={handleSubmit}>
            <h1 className='text-gray-800 md:text-2xl my-12'>Binary Labs Saturday Session | <span className='text-green-800 font-bold'>SignUp</span></h1>
            ...
            <input type="password" onChange={handleChange} name="confirm" id="confirm" placeholder='Confirm password' className='mb-2 md:w-4/5 rounded px-8 py-2 border-2 outline-none focus:border-green-400' />
            <button type='submit' disabled={isLoading ? true : false} className='mt-8 bg-green-600 w-full md:w-4/5 py-2 rounded text-gray-200 font-bold transition ease-in-out duration-700 hover:opacity-80'>{isLoading? <Loading /> :'Register'}</button>
